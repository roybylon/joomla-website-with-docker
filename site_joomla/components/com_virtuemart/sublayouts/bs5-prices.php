...
?>
<div class="product-price mb-auto pb-3" id="productPrice<?php echo $product->virtuemart_product_id ?>" data-vm="product-prices">
	<?php if ($product->prices['salesPrice']<=0 and VmConfig::get ('askprice', 1) and isset($product->images[0]) and !$product->images[0]->file_is_downloadable) : ?>
		<?php $askquestion_url = Route::_('index.php?option=com_virtuemart&view=productdetails&task=askquestion&virtuemart_product_id=' . $product->virtuemart_product_id . '&virtuemart_category_id=' . $product->virtuemart_category_id . '&tmpl=component', FALSE); ?>
	...
	<?php else : ?>
		<?php if (!vmConfig::get('salesPriceText', 0)) : ?>
			<div class="vm-simple-price-display d-flex justify-content-center align-items-center<?php echo $product->prices['discountAmount'] ? ' vm-has-discount' : ''?>">
				<?php
		...
		<?php else : ?>
			...
			<?php if ($infotax == 1 or $infodelivery == 1) : ?>
