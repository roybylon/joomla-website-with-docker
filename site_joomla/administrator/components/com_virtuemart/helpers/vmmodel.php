...
class VmModel extends vObject{
	...
	function setIdByRequest(){
		// Get the task
		$id = 0;
		//vmTrace('setIdByRequest');
		//vmdebug('Model '.get_class($this).' $this->_cidName ',$this->_cidName);
		if(!empty($this->_cidName)){
			// Get the id or array of ids.
			$idArray = vRequest::getInt($this->_cidName,  null);
			vmdebug('setIdByRequest',$this->_cidName,$idArray);
			if(isset($idArray)){
				if(is_array($idArray)){
					$id = $this->setId(reset($idArray));
				} else {
					$id = $this->setId($idArray);
				}

			} else {
				if(empty($id) and get_class($this)!='VirtueMartModelUser')vmdebug('Model '.get_class($this).' no id in request '.$this->_cidName);
			}
		} else {
			vmError('Model '.get_class($this).' is missing $this->_cidName');
		}
		return $id;
	}
	 ...
	 * @since   11.1
	 */
	protected function _getList($query, $limitstart = 0, $limit = 0, $key = 0) {
		...
		if(isset($c[$h])){
			...
			if($key === 0){
				...
				$c[$h] = $this->_db->loadObjectList($key);
			}
...
// 			vmdebug('Get standard model of the view');
	...
	function setDefaultValidOrderingFields($defaultTable=null){
		...
		foreach ($dTableArray as $k => $v){
			...
			if (property_exists($defaultTable, $k)) {
	...
	function checkFilterDir($toCheck, $view = 0, $task = ''){
		...
		if(empty($toCheck)){
			vmdebug('checkFilterDir $toCheck empty');
	...
	public function setPaginationLimits($force = false){
		...
		//vmdebug('setPaginationLimits  limitstart ',$this->_limitStart,$this->_limit);
