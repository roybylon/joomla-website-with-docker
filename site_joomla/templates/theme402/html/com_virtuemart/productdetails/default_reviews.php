...
if ($this->allowRating || $this->showReview) {
	...
	$ratingWidth = $maxrating * 19;
	for ($num = 0; $num <= $maxrating; $num++) {
		$stars[] = '
				<span title="' . (JText::_ ("COM_VIRTUEMART_RATING_TITLE") . $num . '/' . $maxrating) . '" class="vmicon ratingbox" style="display:inline-block;width:' . 19 * $maxrating . 'px;">
					<span class="stars-orange" style="width:' . (19 * $num) . 'px">
					</span>
				</span>';
	} ?>
	...
	if ($this->allowReview) {
		...
		<div class="write-reviews">
			...
			if ($review_editable) {
				...
				echo '<strong>' . JText::_ ('COM_VIRTUEMART_DEAR') . $this->user->name . ',</strong>';
